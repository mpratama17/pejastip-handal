-- Kolom order_id bentrok dengan kolom OUT fungsi (returns table order_id) →
-- "column reference order_id is ambiguous" saat ada buku manual. Qualify alias.
  ...
  if v_order_id is null then
    ...
    loop
      ...
      if v_item ? 'event_item_id' then
        ...
        if exists (select 1 from order_items oi where oi.order_id = v_order_id and oi.event_item_id = v_ei.id) then
