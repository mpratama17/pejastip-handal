...
- **Halaman admin**: ✅ Dashboard (kartu bisa diklik), **Event** (buat/edit, jadwal buka/tutup ditegakkan di `create_order`, konfirmasi status yang menggeser buku), Katalog + CSV, Order + detail + **order manual** (batch tanpa katalog / order dari WA; buku manual disimpan nonaktif sehingga tidak tampil di katalog publik; tombol kirim kode via WA), **Pembayaran** (pratinjau bukti via signed URL, verifikasi dengan koreksi nominal, tolak wajib alasan), **Pengiriman** (resi/ongkir/layanan, tandai diterima, salin alamat), **Customer** (piutang, riwayat order, catatan, blacklist wajib alasan — ditegakkan constraint DB), **Request Buku**, **Pengaturan** (toko, WA/rekening, kurir, DP default, S&K), **detail order**: edit buku (qty/tambah/hapus, buku yang sudah diproses kirim terkunci, harga lama dipertahankan) + override status per buku (R15/R17) — diuji di browser lalu dikembalikan ke seed.
...
- **Git**: https://github.com/mpratama17/pejastip-handal — branch `main`, `ui`, `data`, `dev/backend`. Alur fitur: kerjakan di `dev/backend` (fullstack) / `ui` (UI saja) → uji → PR ke `main` → merge.
